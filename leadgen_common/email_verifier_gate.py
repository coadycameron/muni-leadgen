...
class VerifaliaClient:
    ...
    def __init__(self) -> None:
        ...
        self.base_url = os.environ.get("VERIFALIA_BASE_URL", "https://api-1.verifalia.com/v2.7").rstrip("/")
        ...
        self.batch_size = int(os.environ.get("VERIFALIA_BATCH_SIZE", "50"))
    ...
    def _post_job(self, emails: List[str]) -> Tuple[int, Dict, Optional[str]]:
        ...
        if r.status_code == 402:
            raise RuntimeError("Verifalia returned 402 (credits likely depleted).")
    ...
    def _get_job_snapshot(self, job_id: str) -> Tuple[int, Dict]:
        ...
        with httpx.Client(timeout=self.timeout_s, auth=self._auth()) as client:
            ...
            if r.status_code == 402:
                raise RuntimeError("Verifalia returned 402 while polling (credits depleted).")
            r.raise_for_status()
            return r.status_code, {}
        ...
        if self.cache_enabled and self.cache is not None:
            ...
            for e, doc in docs.items():
                if not self.cache.is_fresh(doc):
                    continue
                if self.cache.is_zb_valid(doc) or self.cache.is_vf_valid(doc):
